...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for IBM Informix (beta)](https://www.cdata.com/download/download.aspx?sku=JIZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data IBM Informix supported by the [CData JDBC Driver for IBM Informix](https://www.cdata.com/drivers/ibminformix/jdbc).
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/ibminformix/download/jdbc](https://www.cdata.com/drivers/ibminformix/download/jdbc)
3. License the CData JDBC Driver:
    ...
    * Run the command `java -jar cdata.jdbc.informix.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.informix.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `ibm-informix.prp`) using the following properties and format:
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "ibm_informix_get_tables",
        "arguments": {}
    }
}
```

#### ibm_informix_get_columns
```json
{
    "jsonrpc": "2.0",
    "id": 2,
